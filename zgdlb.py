...
# 全局线程池配置
MAX_WORKERS = 5  # 控制并发线程数
REQUEST_DELAY = 0.5  # 请求之间的最小延迟(秒)
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for detail_pub in detail_pubs:
                try:
                    ...
                    # 提交附件下载任务
                    futures.append(executor.submit(download_attachment_task, detail_pub, article_dir))
                    
                    # 提交详细数据处理任务
                    futures.append(executor.submit(process_detail_data_task, zb_guid, article_dir))
...
def download_attachment_task(detail_pub, article_dir):
    """下载附件任务"""
...
def process_detail_data_task(zb_guid, article_dir):
    """处理详细数据任务"""
    if not process_detail_data(zb_guid, article_dir):
        print(f"处理文章详情失败: {zb_guid}")
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for obj in menu_pubs:
                try:
                    ...
                    # 提交附件下载任务
                    futures.append(executor.submit(download_menu_attachments, obj, section_dir))
                    
                    # 提交详细发布信息处理任务
                    futures.append(executor.submit(process_bm_detail_pub, ircatelog, doc_pub_time, section_dir))
...
def download_menu_attachments(obj, section_dir):
    """下载菜单附件"""
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for date in date_range:
                ...
                # 提交处理任务
                futures.append(executor.submit(process_date_data, date_str, date_dir))
...
def process_date_data(date_str, date_dir):
    """处理单日数据"""
