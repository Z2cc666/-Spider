...
GZH_LIST = [
    {
        "name": "赛柏蓝",
        "token": "134791494",
        "cookie": "RK=DC2Uq4Wf9P; ptcz=c9f4dcf0c0fb279d2316b228ce1d2d7a6b107f591ae8bbce0eac0ce98bc9de36; wxuin=51340895845860; mm_lang=zh_CN; _hp2_id.1405110977=%7B%22userId%22%3A%226801447023479475%22%2C%22pageviewId%22%3A%228306667787246811%22%2C%22sessionId%22%3A%224504468753015668%22%2C%22identity%22%3Anull%2C%22trackerVersion%22%3A%224.0%22%7D; uuid=83c08156fcc8074b379d8529fd3ca8a9; xid=613a60102783853248452d4c90369541; rand_info=CAESIAV0Gj1aWzFQcQ6V+rjd8fPhQZjtUoCKr53pgYJ5unzt; slave_user=gh_b3cdf815ccbf; slave_sid=U1Vfam5HV0NXNzRDVlY5OV96aFJ3eEFLOV9EQTVrMVVObzZJNzhNam83bWhJU0thb0VIV001RnBMQ0VNU0pzQ00wZjVZZEpNUDlfZTdoWnhFTG1Ud2hOUHZPbmZ2V2xsQnowV2JtelhuNmhtSklWQ1ppbm5FSFJVbWMyWFBCbFJXUnZqMXNIbXZEZFl0QWF1; slave_bizuin=3911425635; data_bizuin=3911425635; ua_id=mxGDXOVuOo8d0D2hAAAAACdqUxp53FqemlDjGf2eSLM=; bizuin=3911425635; data_ticket=BH4rPGbEEySu6B+LVMYw9rJddgR9ddfqpg1Q5yCFKPqnrvKd33WtSqLSyjlg9/u9; rewardsn=; wxtokenkey=777; poc_sid=HBg3iGijlmGc_2ocHEPN26JgrEcR59UETkMwwy7P; _clck=3911425635|1|fy2|0; _clsk=1lqrtr|1753962768289|1|1|mp.weixin.qq.com/weheat-agent/payload/record"
    },
]
...
# 性能配置
PERFORMANCE_CONFIG = {
    'max_workers': 5,  # 并发线程数，可根据网络情况调整（建议3-8）
    'timeout': 10,     # 请求超时时间（秒）
    'page_delay': 0.5, # 页面间延迟（秒）
...
def fetch_articles_batch(gzh, start_page=0, batch_size=20, page_size=10):
    ...
    for page in range(start_page, end_page):
        ...
        headers = {
            'cookie': gzh['cookie'],
            'referer': 'https://mp.weixin.qq.com/cgi-bin/appmsg?t=media/appmsg_edit_v2&action=edit&isNew=1&type=77&createType=0&token=134791494&lang=zh_CN&timestamp=1753954904333',
            'accept': '*/*',
        ...
        try:
            ...
            # 添加调试信息
            if page == 0:  # 只在第一页显示详细信息
                print(f"API响应结构: {list(result.keys())}")
                if "publish_page" in result:
                    print(f"publish_page结构: {list(result['publish_page'].keys())}")
            ...
            # 提取文章列表 - 参考gzh_illegal_spider.py的正确做法
            publish_page = result.get("publish_page")
            if not publish_page:
                print(f"第{page+1}页无publish_page数据")
                consecutive_empty_pages += 1
                if consecutive_empty_pages >= max_consecutive_empty:
            ...
            # 关键：publish_page是字符串，需要json.loads()解析
            try:
                articleList = json.loads(publish_page)
                publish_list = articleList.get("publish_list", [])
            ...
            print(f"第{page+1}页获取到 {len(publish_list)} 个发布信息，提取到 {len([a for a in all_articles if a])} 篇文章")
...
def extract_name(text):
    ...
    invalid_words = [
        # 基础无效词
        '赛柏蓝', '医药慧', '医院', '主任医师', '副主任医师', '主治医师', '护师', '董事长', '副总裁',
...
def extract_hospital(text):
    ...
    government_patterns = [
        ...
        r'([\u4e00-\u9fa5]{2,8}[省市县区](?:卫生局|医保局|药监局|税务局|工商局|公安局|人社局|民政局|财政局|审计局|司法局|自然资源局|生态环境局|住建局|交通运输局|水利局|农业农村局|商务局|文旅局|应急管理局|市场监管局|统计局|医疗保障局|机关事务管理局))',
        # 各种厅
...
def filter_and_extract(articles, gzh_name, existing_results=None, max_workers=None):
    ...
    for article in articles:
        try:
            ...
            # 提前进行标题筛选
            if title and quick_title_filter(title):
        ...
        try:
            ...
            if i < 3:
                ...
                for j, v in enumerate(violations):
                    print(f"  事件{j+1}: 姓名={v['姓名']}, 医院={v['医院']}, 职位={v['职位']}")
                    print(f"           描述={v['描述'][:100]}...")
...
def save_progress(current_page, gzh_name, all_results):
    ...
    progress_file = '微信公众号文章/赛柏蓝_爬取进度.json'
    ...
    if all_results:
        ...
        output_file = '微信公众号文章/赛柏蓝违法事件.xlsx'
...
def load_progress():
    """加载爬取进度"""
    progress_file = '微信公众号文章/赛柏蓝_爬取进度.json'
    data_file = '微信公众号文章/赛柏蓝违法事件.xlsx'
...
def main():
    ...
    for gzh in GZH_LIST:
        ...
        if not fakeid:
            print(f"❌ 无法获取{gzh['name']}的fakeid，跳过该公众号")
            continue
        gzh['fakeid'] = fakeid
        print(f"✅ 成功获取fakeid: {fakeid[:20]}...")
        
        # 批量处理：每20页抓取一次，然后筛选
        current_page = start_page
        max_pages = 1000  # 最大页数
        batch_size = 20   # 每批次20页
