...
class EconomicDailySpider:
    ...
    def process_image(self, img, article_url, article_dir, title, date_str, version_code, saved_images):
        ...
        try:
            ...
            # 过滤装饰性图片
            if any(skip in img_url.lower() for skip in ['ico', 'logo', 'banner']):
                return
            ...
            if not img_url.startswith('http'):
                if img_url.startswith('//'):
                    ...
                    img_url = 'http://paper.ce.cn' + img_url
                else:
                    ...
                    for img_url in possible_urls:
                        if img_url not in saved_images:
                            try:
                                ...
                                print(f"      尝试备选图片URL失败: {img_url}, 错误: {str(e)}")
