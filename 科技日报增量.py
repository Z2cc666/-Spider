...
class ScienceDailyIncrementalSpider:
    def __init__(self):
        self.headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
            'Referer': 'https://digitalpaper.stdaily.com/',
        ...
        self.base_url = "https://digitalpaper.stdaily.com/http_www.kjrb.com/kjrb/html"
        self.base_dir = "科技日报"
        self.request_delay = 2
        
        # 记录文件
        self.record_file = "科技日报_crawled_articles.json"
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('科技日报_crawler.log', encoding='utf-8'),
    ...
    def get_main_page(self, date_str):
        ...
        try:
            ...
            # 尝试多个可能的主页面URL
            possible_urls = [
                f"{self.base_url}/{year}-{month}/{day}/node_2.htm",
                f"{self.base_url}/{year}-{month}/{day}/node_1.htm",
                f"{self.base_url}/{year}-{month}/{day}/index.htm",
                f"{self.base_url}/{year}-{month}/{day}/node_index.htm",
            ]
            
            for url in possible_urls:
                self.logger.info(f"尝试访问主页面: {url}")
                resp = self.safe_request(url)
                if resp:
                    self.logger.info(f"成功访问主页面: {url}")
                    return resp, url
                    
            self.logger.error(f"无法访问 {date_str} 的任何主页面")
            return None, None
    ...
    def get_version_list(self, date_str):
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                # 检查是否是版面链接
                if ('node_' in href or 'page_' in href) and any(p in text for p in ['版', '第']):
                    for pattern in patterns:
                        match = re.search(pattern, text)
                        if match:
                            code = match.group(1).zfill(2)
                            name = match.group(2).strip() if len(match.groups()) > 1 else "未知版面"
                            
                            # 构建完整URL
                            if href.startswith('/'):
                                version_url = f"https://digitalpaper.stdaily.com{href}"
            ...
            if not versions:
                ...
                for i in range(1, 13):
                    code = str(i).zfill(2)
                    possible_urls = [
                        f"{self.base_url}/{year}-{month}/{day}/node_{code}.htm",
                        f"{self.base_url}/{year}-{month}/{day}/page_{code}.htm",
                        f"{self.base_url}/{year}-{month}/{day}/node_{i}.htm",
                        f"{self.base_url}/{year}-{month}/{day}/page_{i}.htm",
                    ]
    ...
    def get_articles_from_version(self, version_url):
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                # 检查是否是文章链接
                if (('content_' in href or 'article_' in href or 'news_' in href) and 
                    title and len(title) > 2 and title not in processed):
                    
                    # 过滤掉导航链接
                    if any(skip in title for skip in ['上一版', '下一版', '返回目录', '科技日报', '首页', '新闻', '图片']):
                        continue
                    
                    # 过滤掉旧的文章链接（2008-2009年的）
                    if any(year in href for year in ['2008', '2009']):
                        continue
                    
                    # 构建完整URL
                    if href.startswith('/'):
                        full_url = f"https://digitalpaper.stdaily.com{href}"
                    elif href.startswith('http'):
                        full_url = href
                    else:
    ...
    def save_images(self, soup, article_dir, title, article_url):
        ...
        for img in soup.find_all('img'):
            ...
            # 构建完整的图片URL
            if src.startswith('/'):
                img_url = f"https://digitalpaper.stdaily.com{src}"
            ...
            try:
                ...
                if img_resp and img_resp.content and len(img_resp.content) > 5000:  # 至少5KB
                    ...
                    if '.' in img_url.split('/')[-1]:
                        ...
                        if potential_ext in ['.jpg', '.jpeg', '.png', '.bmp']:  # 不包含.gif
                            ext = potential_ext
...
def run_spider():
    """运行爬虫的定时任务"""
    spider = ScienceDailyIncrementalSpider()
    spider.crawl_today()
...
if __name__ == '__main__':
    ...
    if TEST_MODE:
        ...
        schedule.every().day.at("16:40").do(run_spider)  # 下午3点32分
        schedule.every().day.at("18:00").do(run_spider)  # 下午6点
        schedule.every().day.at("22:00").do(run_spider)  # 晚上10点
        
        print(f"增量爬虫已启动，将在每天 06:00、10:00、15:32、18:00、22:00 运行...")
